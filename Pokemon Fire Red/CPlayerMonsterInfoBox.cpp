...
CPlayerMonsterInfoBox::CPlayerMonsterInfoBox()
	: m_pImage(nullptr)
	, m_Font(nullptr)
	, m_Expbar(nullptr)
	, m_pRedHPSound(nullptr)
	, m_wsName{}
	, m_MaxHP(0)
	, m_CurHP(0)
	, m_Level(0)
	...
	, m_bSoundON(false)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"PlayerMonsterInfoBox");
	m_Font = CResourceMgr::CreateMgr()->FindImg(L"InfoBoxFont");
	m_HPbar = CResourceMgr::CreateMgr()->FindImg(L"GreenHPbar");
	m_Expbar = CResourceMgr::CreateMgr()->FindImg(L"Expbar");
	m_pRedHPSound = CResourceMgr::CreateMgr()->FindSound(L"RedHP");
}
...
void CPlayerMonsterInfoBox::tick()
{
...
void CPlayerMonsterInfoBox::render(HDC _dc)
{
	Vec2 vPos = GetPos();
	...
	TemHPPos.x += 300;
	TemHPPos.y += 115;
	if (std::to_wstring(m_CurHP).size() == 1)
	{
		m_wsHP += L"  ";
	}
	else if (std::to_wstring(m_CurHP).size() == 2)
	{
		m_wsHP += L" ";
	}
	m_wsHP += std::to_wstring(m_CurHP);
	m_wsHP += L"/";

	if (std::to_wstring(m_MaxHP).size() == 1)
	{
		m_wsHP += L"  ";
	}
	else if (std::to_wstring(m_MaxHP).size() == 2)
	{
		m_wsHP += L" ";
	}
	m_wsHP += std::to_wstring(m_MaxHP);
	...
	Vec2 TemExpbarPos = vPos;
	TemExpbarPos.x += 160;
	TemExpbarPos.y += 165;
	float Expratio = (float)m_CurExp / (float)m_MaxExp;

	AlphaBlend(_dc, TemExpbarPos.x
		, TemExpbarPos.y
		, (m_Expbar->GetWidth() * Expratio), m_Expbar->GetHeight()
		, m_Expbar->GetDC(), 0, 0, (m_Expbar->GetWidth() * Expratio), m_Expbar->GetHeight(), tFunc);

	CUI::render(_dc);
}

Fint CPlayerMonsterInfoBox::LinkText(char _text)
{
	switch (_text)
	{
	case 'A':
