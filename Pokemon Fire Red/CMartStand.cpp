...
CMartStand::CMartStand()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"MartStand");

	AddCollider(L"Image\\map\\Environment\\Inside\\MartStand.Collider");
}
...
void CMartStand::tick()
{
	CEnvironment::tick();
}

void CMartStand::render(HDC _dc)
{
...
void CMartStand::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CMartStand::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CMartStand::CollisionEndOverlap(CCollider* _pOtherCollider)
{
