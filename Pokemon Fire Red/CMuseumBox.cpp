...
CMuseumBox::CMuseumBox()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"MuseumBox");

	AddCollider(L"Image\\map\\Environment\\Inside\\MuseumBox.Collider");
}
...
void CMuseumBox::tick()
{
	CEnvironment::tick();
}

void CMuseumBox::render(HDC _dc)
{
...
void CMuseumBox::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CMuseumBox::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CMuseumBox::CollisionEndOverlap(CCollider* _pOtherCollider)
{
