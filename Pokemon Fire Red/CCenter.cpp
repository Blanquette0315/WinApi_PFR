...
CCenter::CCenter()
	...
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"Center");

	AddCollider(L"Image\\map\\Building\\Center.Collider");
}
...
void CCenter::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CCenter::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CCenter::CollisionEndOverlap(CCollider* _pOtherCollider)
{
