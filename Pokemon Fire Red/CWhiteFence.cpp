...
CWhiteFence::CWhiteFence()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"WhiteFence");

	AddCollider(L"Image\\map\\Environment\\WhiteFence.Collider");
}
...
void CWhiteFence::tick()
{
	CEnvironment::tick();
}

void CWhiteFence::render(HDC _dc)
{
...
void CWhiteFence::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CWhiteFence::CollisionOverlap(CCollider* _pOtherCollider)
{
	
}

void CWhiteFence::CollisionEndOverlap(CCollider* _pOtherCollider)
{
