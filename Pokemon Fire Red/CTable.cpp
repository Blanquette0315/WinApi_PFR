...
CTable::CTable()
	...
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"Table");

	AddCollider(L"Image\\map\\Environment\\Inside\\Table.Collider");
}
...
void CTable::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CTable::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CTable::CollisionEndOverlap(CCollider* _pOtherCollider)
{
