...
CCenterSofa::CCenterSofa()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"CenterSofa");

	AddCollider(L"Image\\map\\Environment\\Inside\\CenterSofa.Collider");
}
...
void CCenterSofa::tick()
{
	CEnvironment::tick();
}

void CCenterSofa::render(HDC _dc)
{
...
void CCenterSofa::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CCenterSofa::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CCenterSofa::CollisionEndOverlap(CCollider* _pOtherCollider)
{
