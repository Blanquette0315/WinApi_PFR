...
CMartTable::CMartTable()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"MartTable");

	AddCollider(L"Image\\map\\Environment\\Inside\\MartTable.Collider");
}
...
void CMartTable::tick()
{
	CEnvironment::tick();
}

void CMartTable::render(HDC _dc)
{
...
void CMartTable::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CMartTable::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CMartTable::CollisionEndOverlap(CCollider* _pOtherCollider)
{
