...
{
	if (nullptr == CSoundMgr::CreateMgr()->GetSoundDevice())
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(CSoundMgr::CreateMgr()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
{
	CSoundMgr::CreateMgr()->RegisterToBGM(this);
