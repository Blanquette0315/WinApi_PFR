...
COakBookshelfHalf::COakBookshelfHalf()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"OakBookshelfHalf");

	AddCollider(L"Image\\map\\Environment\\Inside\\OakBookshelfHalf.Collider");
}
...
void COakBookshelfHalf::tick()
{
	CEnvironment::tick();
}

void COakBookshelfHalf::render(HDC _dc)
{
...
void COakBookshelfHalf::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void COakBookshelfHalf::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void COakBookshelfHalf::CollisionEndOverlap(CCollider* _pOtherCollider)
{
