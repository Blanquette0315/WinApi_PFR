...
CCurPokemonBox::CCurPokemonBox(CPokemon* _Pokemon)
	: m_pImage(nullptr)
	...
	, m_bAnim(true)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"CurPokemonBox");
	m_pTargetImage = CResourceMgr::CreateMgr()->FindImg(L"CurPokemonBoxTarget");
	m_pFont = CResourceMgr::CreateMgr()->FindImg(L"PokemonBoxFont");
}

CCurPokemonBox::~CCurPokemonBox()
{
...
void CCurPokemonBox::render(HDC _dc)
{
	Vec2 vPos = GetPos();
	...
	if (m_bIS_Target)
	{
		AlphaBlend(_dc, vPos.x - m_pTargetImage->GetWidth()
			, vPos.y - m_pTargetImage->GetHeight()
...
Fint CCurPokemonBox::LinkText(char _text)
{
	switch (_text)
	{
	case 'A':
