...
CATree::CATree()
	...
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"ATree");

	AddCollider(L"Image\\map\\Environment\\ATree.Collider");
}
...
void CATree::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CATree::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CATree::CollisionEndOverlap(CCollider* _pOtherCollider)
{
