...
CCenterTable::CCenterTable()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"CenterTable");

	AddCollider(L"Image\\map\\Environment\\Inside\\CenterTable.Collider");
}
...
void CCenterTable::tick()
{
	CEnvironment::tick();
}

void CCenterTable::render(HDC _dc)
{
...
void CCenterTable::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CCenterTable::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CCenterTable::CollisionEndOverlap(CCollider* _pOtherCollider)
{
