...
CMuseumStand::CMuseumStand()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"MuseumStand");

	AddCollider(L"Image\\map\\Environment\\Inside\\MuseumStand.Collider");
}
...
void CMuseumStand::tick()
{
	CEnvironment::tick();
}

void CMuseumStand::render(HDC _dc)
{
...
void CMuseumStand::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CMuseumStand::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CMuseumStand::CollisionEndOverlap(CCollider* _pOtherCollider)
{
