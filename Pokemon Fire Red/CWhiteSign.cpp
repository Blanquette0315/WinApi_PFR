...
CWhiteSign::CWhiteSign()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"WhiteSign");

	AddCollider(L"Image\\map\\Environment\\WhiteSign.Collider");
}
...
void CWhiteSign::tick()
{
	CEnvironment::tick();
}

void CWhiteSign::render(HDC _dc)
{
...
void CWhiteSign::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CWhiteSign::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CWhiteSign::CollisionEndOverlap(CCollider* _pOtherCollider)
{
