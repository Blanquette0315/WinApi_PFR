...
COakBookShelf::COakBookShelf()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"OakBookshelf");

	AddCollider(L"Image\\map\\Environment\\Inside\\OakBookshelf.Collider");
}
...
void COakBookShelf::tick()
{
	CEnvironment::tick();
}

void COakBookShelf::render(HDC _dc)
{
...
void COakBookShelf::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void COakBookShelf::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void COakBookShelf::CollisionEndOverlap(CCollider* _pOtherCollider)
{
