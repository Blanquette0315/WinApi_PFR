...
COaksLab::COaksLab()
	...
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"OaksLab");

	AddCollider(L"Image\\map\\Building\\OaksLab.Collider");
}
...
void COaksLab::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
	// Render 기준 Collider 충돌 해제 시 IS_Collision true로 변경
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
		if (!(0 == GetCollider(1)->GetCollisionCount())
			&& (0 == m_iRe_render))
		{
			m_iRe_render++;
			IS_Collision = true;
	...
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
		...
		{
			SceneChange(SCENE_TYPE::OAKLAB);
		}
	}
}

void COaksLab::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void COaksLab::CollisionEndOverlap(CCollider* _pOtherCollider)
{
