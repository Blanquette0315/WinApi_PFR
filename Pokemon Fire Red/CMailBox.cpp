...
CMailBox::CMailBox()
	...
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"MailBox");

	AddCollider(L"Image\\map\\Environment\\MailBox.Collider");
}
...
void CMailBox::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CMailBox::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CMailBox::CollisionEndOverlap(CCollider* _pOtherCollider)
{
