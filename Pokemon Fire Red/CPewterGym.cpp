...
CPewterGym::CPewterGym()
	: m_pImage(nullptr)
	, IS_Collision(false)
	, m_iRe_render(0)
{
	m_pImage = CResourceMgr::CreateMgr()->FindImg(L"PewterGym");

	AddCollider(L"Image\\map\\Building\\PewterGym.Collider");
}
...
void CPewterGym::tick()
{
	CEnvironment::tick();
}

void CPewterGym::render(HDC _dc)
{
...
void CPewterGym::CollisionBeginOverlap(CCollider* _pOtherCollider)
{
...
void CPewterGym::CollisionOverlap(CCollider* _pOtherCollider)
{
	// ImegeRender
	if (dynamic_cast<CPlayer*>(_pOtherCollider->GetOwner()))
	{
...
void CPewterGym::CollisionEndOverlap(CCollider* _pOtherCollider)
{
